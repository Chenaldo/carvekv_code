"""
final_run2.py  --  DeepSeek-Coder-V2-Lite Latent KV Eviction
              2×2 Factorial Ablation Study
=======================================================================

Goal
----
  Decompose the PPL / recall improvement observed in the committee eviction
  algorithm into two orthogonal contributions:

    Factor A — Scoring strategy
      single   : only layer-16 attention scores determine token importance
      committee: layers {4,6,8,12,16} vote; layer 16 takes the average
                 (multi-layer consensus reduces score noise)

    Factor B — Eviction strategy
      hard  : evicted tokens are permanently dropped (pool_ratio=0)
      super : evicted tokens are pooled into super tokens and kept in
              the cache (pool_ratio=4 → 1 super per 4 evicted tokens)

Experiment matrix (all eviction configs use cov_floor=0.90)
-----------------------------------------------------------
                    | single scoring  | committee scoring |
  ------------------+-----------------+-------------------+
  hard eviction     | single_hard     | committee_hard    |
  super tokens      | single_super    | committee_super   |
  ------------------+-----------------+-------------------+
  + baseline (no eviction, oracle upper bound)

How to read the ablation table printed at the end
-------------------------------------------------
  Δ_committee  = committee_hard  − single_hard   (pure committee benefit)
  Δ_super      = single_super    − single_hard   (pure super-token benefit)
  Δ_interaction= committee_super − committee_hard
                                − single_super + single_hard
  (For PPL: negative Δ = better. For recall/score: positive Δ = better.)

Test categories (6)
-------------------
  1. Suffix PPL     code-search-net/python → fineweb-edu(code) → built-in Python
  2. NIAH           Needle-in-a-Haystack at 25%%/50%%/75%% depth, 2k/4k/8k ctx
  3. Code recall    6 cross-reference prompts
  4. HumanEval      syntax-pass@1  (optional)
  5. LongBench      multi-doc QA F1  (optional)
  6. System         VRAM / prefill latency / TTFT / cache compression

Usage
-----
    python final_run2.py                     # full run
    python final_run2.py --quick             # fast smoke test (~15 min)
    python final_run2.py --skip-optional     # skip HumanEval + LongBench
    python final_run2.py --niah-max 32768    # extend NIAH to 32k tokens
    python final_run2.py --save out.json     # output path
"""
...
# ── Experiment configurations — 2×2 Factorial Ablation ──────────────────────
#
#   Factor A: scoring strategy  — single layer 16  vs  committee {4,6,8,12,16}
#   Factor B: eviction strategy — hard drop (pool_ratio=0)  vs  super tokens (pool_ratio=4)
#
#   All four eviction cells use cov_floor=0.90 so compression pressure is
#   identical, making A/B effects directly comparable.
#
#   ┌─────────────────┬──────────────────┬───────────────────┐
#   │                 │ single scoring   │ committee scoring │
#   ├─────────────────┼──────────────────┼───────────────────┤
#   │ hard eviction   │ single_hard      │ committee_hard    │  ← pool_ratio=0
#   │ super tokens    │ single_super     │ committee_super   │  ← pool_ratio=4
#   └─────────────────┴──────────────────┴───────────────────┘
#   + baseline (eviction=False) as the oracle upper bound
#
#   Reading the ablation:
#     Δ_committee  = committee_hard  - single_hard    (pure multi-layer gain)
#     Δ_super      = single_super    - single_hard    (pure super-token gain)
#     Δ_interaction= committee_super - committee_hard
#                                    - single_super + single_hard
CONFIGS: List[Dict[str, Any]] = [
    ...
    # ── Cell (single, hard): legacy single-layer hard-eviction reference ──────
    {"name": "single_hard",
     "eviction": True,  "decision_layer": 16,
     "score_layers": [16], "cov_floor": 0.90, "pool_ratio": 0},

    # ── Cell (single, super): isolates super-token contribution ───────────────
    {"name": "single_super",
     "eviction": True,  "decision_layer": 16,
     "score_layers": [16], "cov_floor": 0.90, "pool_ratio": 4},

    # ── Cell (committee, hard): isolates committee-scoring contribution ────────
    {"name": "committee_hard",
     "eviction": True,  "decision_layer": 16,
     "score_layers": [4, 6, 8, 12, 16], "cov_floor": 0.90, "pool_ratio": 0},

    # ── Cell (committee, super): full algorithm — both improvements combined ───
    {"name": "committee_super",
     "eviction": True,  "decision_layer": 16,
...
def print_summary(all_results: Dict) -> None:
    ...
    W      = 18          # widened to fit longest name "committee_super" (15)
...
def print_ablation_analysis(all_results: Dict) -> None:
    """
    Decompose the observed quality change into:
      Δ_committee  (effect of multi-layer scoring, holding eviction strategy fixed)
      Δ_super      (effect of super tokens,        holding scoring strategy fixed)
      Δ_interaction(synergy / anti-synergy between the two factors)

    The reference cell is single_hard (single-layer + hard drop).
    For PPL: negative Δ means improvement (lower is better).
    For recall / score: positive Δ means improvement (higher is better).
    """
    _N = {c["name"]: c for c in CONFIGS}
    # Only meaningful if all four ablation cells are present
    required = {"single_hard", "single_super", "committee_hard", "committee_super"}
    if not required.issubset({c["name"] for c in CONFIGS}):
        return

    print("\n\n" + "=" * 72)
    print(f"  {'ABLATION DECOMPOSITION':^68}")
    print("=" * 72)
    print(f"  Reference cell : single_hard  (single-layer scoring + hard eviction)")
    print(f"  Sign convention: PPL → negative Δ = better  |  "
          f"recall/score → positive Δ = better")
    print("-" * 72)

    # Helper: extract scalar metrics from all_results
    def _get(section: str, cfg_name: str, key: str,
             sub_key: Optional[str] = None) -> Optional[float]:
        sec = all_results.get(section, {})
        v   = sec.get(cfg_name, {})
        if sub_key:
            v = v.get(key, {})
            return v.get(sub_key)
        return v.get(key)

    # ── 1. Suffix PPL ─────────────────────────────────────────────────────────
    if "ppl" in all_results:
        print("\n  [Suffix PPL — lower is better; Δ < 0 means improvement]")
        print(f"  {'Config':<20}  {'PPL':>7}  {'Δ vs single_hard':>17}")
        ref = _get("ppl", "single_hard", "suffix_ppl")
        for name in ["baseline", "single_hard", "single_super",
                     "committee_hard", "committee_super"]:
            v = _get("ppl", name, "suffix_ppl")
            if v is None:
                continue
            delta_str = "  (reference)" if name == "single_hard" else \
                        f"  {v - ref:+.3f}" if ref is not None else "  N/A"
            print(f"  {name:<20}  {v:>7.3f}{delta_str}")

        if ref is not None:
            sh  = ref
            ss  = _get("ppl", "single_super",    "suffix_ppl") or sh
            ch  = _get("ppl", "committee_hard",   "suffix_ppl") or sh
            cs  = _get("ppl", "committee_super",  "suffix_ppl") or sh
            d_committee   = ch - sh
            d_super       = ss - sh
            d_interaction = cs - ch - ss + sh
            d_total       = cs - sh
            print(f"\n  Δ_committee   = {d_committee:+.3f}  "
                  f"({'↓ better' if d_committee < 0 else '↑ worse'})")
            print(f"  Δ_super_token = {d_super:+.3f}  "
                  f"({'↓ better' if d_super < 0 else '↑ worse'})")
            print(f"  Δ_interaction = {d_interaction:+.3f}  "
                  f"({'synergy ↓' if d_interaction < 0 else 'anti-synergy ↑'})")
            print(f"  Δ_total       = {d_total:+.3f}  "
                  f"(single_hard → committee_super)")
            total_abs = abs(d_committee) + abs(d_super) + abs(d_interaction)
            if total_abs > 1e-6:
                print(f"  Contribution breakdown (|Δ| share):")
                print(f"    committee   {abs(d_committee)/total_abs:>6.1%}")
                print(f"    super_token {abs(d_super)/total_abs:>6.1%}")
                print(f"    interaction {abs(d_interaction)/total_abs:>6.1%}")

    # ── 2. NIAH recall (average across all tested context lengths) ────────────
    if "niah" in all_results:
        print("\n  [NIAH recall — higher is better; Δ > 0 means improvement]")
        ref_vals = {}
        cfg_avgs: Dict[str, float] = {}
        for name in ["single_hard", "single_super",
                     "committee_hard", "committee_super", "baseline"]:
            ctxd = all_results["niah"].get(name, {})
            recalls = [v.get("recall") for v in ctxd.values()
                       if v.get("recall") is not None]
            if recalls:
                cfg_avgs[name] = float(np.mean(recalls))
        ref = cfg_avgs.get("single_hard")
        print(f"  {'Config':<20}  {'Avg recall':>10}  {'Δ vs single_hard':>17}")
        for name in ["baseline", "single_hard", "single_super",
                     "committee_hard", "committee_super"]:
            v = cfg_avgs.get(name)
            if v is None:
                continue
            delta_str = "  (reference)" if name == "single_hard" else \
                        f"  {v - ref:+.3%}" if ref is not None else "  N/A"
            print(f"  {name:<20}  {v:>10.1%}{delta_str}")

        if ref is not None:
            sh = ref
            ss = cfg_avgs.get("single_super",   sh)
            ch = cfg_avgs.get("committee_hard",  sh)
            cs = cfg_avgs.get("committee_super", sh)
            d_committee   = cs - ch   # using committee_super - committee_hard
            d_super       = ss - sh
            d_committee_main = ch - sh
            d_interaction = cs - ch - ss + sh
            print(f"\n  Δ_committee   = {d_committee_main:+.3%}")
            print(f"  Δ_super_token = {d_super:+.3%}")
            print(f"  Δ_interaction = {d_interaction:+.3%}")

    # ── 3. Code cross-reference recall ────────────────────────────────────────
    if "code_recall" in all_results:
        print("\n  [Code recall — higher is better; Δ > 0 means improvement]")
        ref = _get("code_recall", "single_hard", "score")
        print(f"  {'Config':<20}  {'Score':>7}  {'Δ vs single_hard':>17}")
        for name in ["baseline", "single_hard", "single_super",
                     "committee_hard", "committee_super"]:
            v = _get("code_recall", name, "score")
            if v is None:
                continue
            delta_str = "  (reference)" if name == "single_hard" else \
                        f"  {v - ref:+.3f}" if ref is not None else "  N/A"
        ...
        if ref is not None:
            sh = ref
            ss = _get("code_recall", "single_super",   "score") or sh
            ch = _get("code_recall", "committee_hard",  "score") or sh
            cs = _get("code_recall", "committee_super", "score") or sh
            d_committee   = ch - sh
            d_super       = ss - sh
            d_interaction = cs - ch - ss + sh
            print(f"\n  Δ_committee   = {d_committee:+.3f}")
